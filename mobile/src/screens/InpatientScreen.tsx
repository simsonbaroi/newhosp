...
type InpatientScreenNavigationProp = NativeStackNavigationProp<
  RootStackParamList,
  'Inpatient'
>;

type Props = {
  navigation: InpatientScreenNavigationProp;
};
  ...
  const loadData = async () => {
    try {
      const [items, bills] = await Promise.all([
        ...
        ApiService.getBills('mobile-session', 'inpatient'),
      ]);
    ...
    Alert.alert(
      ...
      [
        {text: 'Cancel', style: 'cancel'},
        {
          ...
          onPress: async () => {
            try {
              await ApiService.clearBills('mobile-session', 'inpatient');
              setBillItems([]);
            } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.container}>
        ...
        <Card style={styles.card}>
          <Card.Content>
            <Title style={styles.sectionTitle}>Inpatient Categories</Title>
